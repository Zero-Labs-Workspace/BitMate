  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    ...
    try {
      if (isTransactionMode) {
        ...
        switch (action) {
          case "transfer":
            if (!isValidWalletAddress(data.result.completion[0].address)) {
              throw new Error("Invalid wallet address");
            }
            const result = await handleTransfer(data.result.completion[0]);
            setMessages([
              ...newMessages,
              {
                role: "bot",
          ...
          case "balance":
            const balance = await handleBalance(data.result.completion[0]);
            setMessages([
              ...newMessages,
              {
                role: "bot",
        ...
        setMessages([
          ...newMessages,
          {
            ...
            content: (
              <ReactMarkdown>
                {data.analysis || "No strategy available for this query."}
              </ReactMarkdown>
            ),
          },
        ]);
        ...
        <div className="space-y-1.5">
          ...
          <div className="flex flex-col">
            <p className="text-sm text-slate-100 opacity-80">
              Intelligent DeFi Assistant for Transactions & Strategy
            </p>
          </div>
        </div>
        <div className="flex items-center gap-x-4">
          ...
          <div className="flex items-center gap-2">
            <Switch
              ...
              className="data-[state=checked]:bg-[#FF9100]"
            />
            <Label className="text-sm font-medium">
              {isTransactionMode ? "Transaction Mode" : "Strategy Mode"}
            </Label>
          </div>
        </div>
      </div>
              ...
              >
                <div
                  className={`p-3 ${
                    role === "user"
                      ? "bg-[#3f3f46]"
                      : "bg-gradient-to-bl from-[#FF9100] to-[#db00e9]"
                  } rounded-full`}
                >
                  {role === "user" ? <User /> : <Bot />}
                </div>
        ...
        <Form {...form}>
          ...
          >
            ...
            <Button
              type="submit"
              disabled={portfolioLoading}
              className={`bg-gradient-to-br from-[#FF9100] to-[#c200e9] rounded-xl ${
